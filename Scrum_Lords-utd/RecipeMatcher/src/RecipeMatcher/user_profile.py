...
def build_user_profile_page(return_to_home_callback):
    ...
    title_label = toga.Label("Log In", style=(LABEL_STYLE))
    ...
    # Submit button
    submit_button = toga.Button("Submit", on_press=lambda x: submit_form(name_input, email_input, password_input, phone_input, address_input), style=BUTTON_STYLE)

    # Clear button
    clear_button = toga.Button("Clear", on_press=lambda x: clear_form(name_input, email_input, password_input, phone_input, address_input), style=BUTTON_STYLE)
